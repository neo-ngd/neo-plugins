...
namespace FSStorageTests.Morph.Event
{
    ...
    public class MorphEventTest
    {
        ...
        public void ParseBindEventTest()
        {
            ...
            Assert.AreEqual(((BindEvent)@event).Keys.Length, 3);
            array.Add(1);
            Action action = () => MorphEvent.ParseNewEpochEvent(array);
            Assert.ThrowsException<Exception>(action);
            @event.ContractEvent();
        ...
        public void ParseUnbindEventTest()
        {
            ...
            Assert.AreEqual(((UnbindEvent)@event).Keys.Length, 3);
            array.Add(1);
            Action action = () => MorphEvent.ParseNewEpochEvent(array);
            Assert.ThrowsException<Exception>(action);
            @event.ContractEvent();
        ...
        public void ParseUpdateInnerRingTest()
        {
            ...
            Assert.AreEqual(((UpdateInnerRingEvent)@event).Keys.Length, 3);
            array.Add(1);
            Action action = () => MorphEvent.ParseNewEpochEvent(array);
            Assert.ThrowsException<Exception>(action);
            @event.ContractEvent();
