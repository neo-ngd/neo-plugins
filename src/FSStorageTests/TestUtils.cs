...
namespace Neo.Plugins.FSStorage.morph.client.Tests
{
    ...
    {
        public readonly static List<RpcTestCase> RpcTestCases = ((JArray)JObject.Parse(File.ReadAllText("MainContractTestCases.json"))).Select(p => RpcTestCase.FromJson(p)).ToList();
    }
